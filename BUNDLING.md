...
# 打包成可执行文件
nexe ./dist/cli.js --target linux-x64-20.20.0 --output ./dist/qwen-linux --resource "./dist/**/*" --resource "./packages/core/dist/**/*" --resource "./packages/cli/dist/**/*"
```
...
1. **nexe 构建时间**：如果使用 `--build` 标志，nexe 需要从源代码编译 Node.js，这可能需要 15-45 分钟。建议在 CI 环境中使用预构建版本。
